...
class OutlierRemover(BaseEstimator, TransformerMixin):
    ...
    def transform(self, df):
        if (set(self.feat_with_outliers).issubset(df.columns)):
            ...
            df = df[
                ~((df[self.feat_with_outliers] < (Q1 - 3 * IQR)) | (df[self.feat_with_outliers] > (Q3 + 3 * IQR))).any(
                    axis=1)]
            return df
        else:
...
class OneHotWithFeatNames(BaseEstimator, TransformerMixin):
    def __init__(self, one_hot_enc_ft=['Gender', 'Marital status', 'Dwelling', 'Employment status', 'Has a car',
                                       'Has a property', 'Has a work phone', 'Has a phone', 'Has an email']):
